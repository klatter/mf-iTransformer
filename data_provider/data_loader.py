...
class Dataset_ETT_hour_MF(Dataset_ETT_hour):
    def __read_data__(self):
        self.scaler = StandardScaler()
        df_raw = pd.read_csv(os.path.join(self.root_path,
                                          self.data_path))
        
        df_raw = df_raw.drop(columns=['date_d1'], errors='ignore')
...
class Dataset_ETT_hour_LF(Dataset_ETT_hour):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='ETTh1_low_1d.csv',
        ...
        super().__init__(root_path, flag=flag, size=size,
