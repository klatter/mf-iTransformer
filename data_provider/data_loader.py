"""Data loader implementations for single- and mixed-frequency datasets.

This module provides dataset classes used by the training scripts. It contains
standard single-frequency dataset implementations (ETT hourly/minute, custom)
and a lightweight mixed-frequency mixin that enables per-group downsampling
and column selection. Mixed datasets return lists of per-frequency tensors
so the experiment adapter can convert them into frequency-keyed dicts.

Key classes:
- `Dataset_ETT_hour`, `Dataset_ETT_minute`, `Dataset_Custom` — single-frequency datasets
- `_MixedSequenceMixin` — helper implementing grouped downsampling and sampling alignment
- `Dataset_*_Mixed` — mixed-frequency dataset variants that return list batches
"""
...
class _MixedSequenceMixin:
    ...
    def _validate_mixed_config(self):
        ...
        if len(self.downsampling_rates) == 1 and len(self.freq_groups) > 1:
            self.downsampling_rates = self.downsampling_rates * len(self.freq_groups)

        if len(self.downsampling_rates) != len(self.freq_groups):
...
class Dataset_Custom_Mixed(_MixedSequenceMixin, Dataset_Custom):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='ETTh1.csv',
                 target='OT', scale=True, timeenc=0, freq='h',
                 downsampling_rates=None, freq_groups=None):
        self._init_mixed_config(downsampling_rates, freq_groups)
        Dataset_Custom.__init__(self, root_path=root_path, flag=flag, size=size,
                                features=features, data_path=data_path,
                                target=target, scale=scale, timeenc=timeenc, freq=freq)
        self._validate_mixed_config()

    def __getitem__(self, index):
        return self._mixed_getitem(index)


class Dataset_ETT_hour_Mixed(_MixedSequenceMixin, Dataset_ETT_hour):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='ETTh1.csv',
                 target='OT', scale=True, timeenc=0, freq='h',
                 downsampling_rates=None, freq_groups=None):
        self._init_mixed_config(downsampling_rates, freq_groups)
        Dataset_ETT_hour.__init__(self, root_path=root_path, flag=flag, size=size,
                                  features=features, data_path=data_path,
                                  target=target, scale=scale, timeenc=timeenc, freq=freq)
        self._validate_mixed_config()
...
class Dataset_ETT_minute_Mixed(_MixedSequenceMixin, Dataset_ETT_minute):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='ETTm1.csv',
                 target='OT', scale=True, timeenc=0, freq='t',
                 downsampling_rates=None, freq_groups=None):
        self._init_mixed_config(downsampling_rates, freq_groups)
        Dataset_ETT_minute.__init__(self, root_path=root_path, flag=flag, size=size,
                                    features=features, data_path=data_path,
                                    target=target, scale=scale, timeenc=timeenc, freq=freq)
        self._validate_mixed_config()
